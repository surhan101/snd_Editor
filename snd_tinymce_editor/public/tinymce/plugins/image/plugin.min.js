...
(function () {
  ...
  const register$2 = editor => {
    ...
    registerOption('image_has_upload_btn', {
      processor: 'boolean',
      default: true
    });
  ...
  const hasUploadBtn = option('image_has_upload_btn');
  ...
  const collect = editor => {
    ...
    const hasUploadBtn$1 = hasUploadBtn(editor);
  ...
  const makeItems = info => {
    const uploadBtn = {
      name: 'upload_img',
      type: 'htmlpanel',
      html: '<button id="upload_editor_img" class="tox-button">Upload</button>',
    };
    const imageUrl = {
      name: 'src',
      type: 'urlinput',
  ...
  const Dialog = editor => {
    ...
    const open = () => {
      ...
      .finally(()=>{
        $("head").append("<style>.modal{z-index: 99999999 !important;} .modal-backdrop{z-index: 9999999 !important;}</style>");
        $('.tox-control-wrap [type="url"]').hide();
        $("#upload_editor_img").click(()=>{
            ...
            new frappe.ui.FileUploader({
              ...
              on_success: (file_doc) => {
                $('.tox-control-wrap [type="url"]').val(file_doc.file_url);
